...
public class EasyGame extends AnimListener {
    ...
    private void CreateEnemies(GL gl) {
        if (choose) {
            ...
            secondEnemy.add(new Enemies2(10 + ((int) (Math.random() * 80)), +20, 2 * 2600));
            choose = true;
        }
    ...
    public void handleKeyPress() {
        if (isKeyPressed(KeyEvent.VK_LEFT)) {
            animationIndex = 2;
            double leftXPlaneBoundary = 0;
            if (planeXPosition > leftXPlaneBoundary) planeXPosition -= plane.getPlaneSpeed();
            if (planeXPosition < leftXPlaneBoundary) planeXPosition = leftXPlaneBoundary;
        } else if (isKeyPressed(KeyEvent.VK_RIGHT)) {
            animationIndex = 1;
            double rightXPlaneBoundary = 100;
            if (planeXPosition < rightXPlaneBoundary) planeXPosition += plane.getPlaneSpeed();
            if (planeXPosition > rightXPlaneBoundary) planeXPosition = rightXPlaneBoundary;
        }
